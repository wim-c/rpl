...
push        sta savey       ; Save acc.
            lda tos         ; Push tos to parameter stack.
            sta pslo,x
            lda tos+1
            sta pshi,x
            dex
...
;
exec_pusha      lda tos         ; Push tos to parameter stack.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
...
; parameter.
;
exec_pushn      lda tos         ; Push tos to parameter stack.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
...
; parameter.
;
exec_pushp      lda tos         ; Push tos to parameter stack.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
...
exec_add        inx
                ...
                lda pshi+1,x
                adc tos+1       ; Add hi bytes of nos and tos.
...
;
iszero          lda tos         ; Pull lo byte.
                ora tos+1       ; Pull hi byte and derive z from (lo OR hi).
                rts
...
exec_divmod     jsr divmod      ; Compute quotient and remainder.
                ...
                sta pshi,x      ; Store remainder as nos.
                tya
                sta pslo,x      ; FIXME: overwrites sign of quotient.
                jsr fixq        ; Load quotient in a;y.
...
; Duplicate tos.
;
exec_dup        lda tos         ; Push tos as nos.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
...
;
exec_fn         lda tos         ; Push tos to nos.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
...
; unsigned byte.
;
exec_get        lda tos         ; Push tos as nos.
                sta pslo,x
                lda tos+1
                sta pshi,x
                dex
